...
class SensorWrapper(object):
    ...
    def _set_attribute_if_present(self, bp, spec, key, default=None):
        """Set a CARLA blueprint attribute when supported."""
        if key not in spec and default is None:
            return

        if hasattr(bp, "has_attribute") and not bp.has_attribute(key):
            return
    ...
    def _configure_camera_attributes(self, bp, spec):
        ...
        self._set_attribute_if_present(bp, spec, "image_size_x")
        self._set_attribute_if_present(bp, spec, "image_size_y")
        self._set_attribute_if_present(bp, spec, "fov")
        self._set_attribute_if_present(bp, spec, "sensor_tick")
    ...
    def _configure_lidar_attributes(self, bp, spec):
        ...
        for key in [
            ...
            self._set_attribute_if_present(bp, spec, key)

    def _configure_gnss_attributes(self, bp, spec):
        """Configure GNSS with zero noise for clean simulation."""
        self._set_attribute_if_present(bp, spec, "sensor_tick")
        for param in ["alt", "lat", "lon"]:
            self._set_attribute_if_present(bp, spec, f"noise_{param}_stddev", 0.0)
            self._set_attribute_if_present(bp, spec, f"noise_{param}_bias", 0.0)

    def _configure_imu_attributes(self, bp, spec):
        """Configure IMU with zero noise for clean simulation."""
        self._set_attribute_if_present(bp, spec, "sensor_tick")
        for axis in ["x", "y", "z"]:
            self._set_attribute_if_present(bp, spec, f"noise_accel_stddev_{axis}", 0.0)
            self._set_attribute_if_present(bp, spec, f"noise_gyro_stddev_{axis}", 0.0)
