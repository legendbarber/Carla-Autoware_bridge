  ...
  size_t current_alignment)
{
  ...
  if (is_plain) {
    ...
    using DataType = autoware_planning_msgs::msg::LaneletPrimitive;
    is_plain =
      (
      offsetof(DataType, primitive_type) +
      last_member_size
      ) == ret_val;
  }
...
static size_t _LaneletPrimitive__max_serialized_size(char & bounds_info)
{
  ...
  ret_val = max_serialized_size_LaneletPrimitive(full_bounded, is_plain, 0);
