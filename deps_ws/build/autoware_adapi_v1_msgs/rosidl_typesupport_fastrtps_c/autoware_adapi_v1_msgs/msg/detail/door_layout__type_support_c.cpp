  ...
  void * untyped_ros_message)
{
  ...
  // Field name: roles
  {
    ...
    if (!rosidl_runtime_c__uint8__Sequence__init(&ros_message->roles, size)) {
      fprintf(stderr, "failed to create array for field 'roles'");
      return false;
    }
    auto array_ptr = ros_message->roles.data;
  ...
  size_t current_alignment)
{
  ...
  if (is_plain) {
    ...
    using DataType = autoware_adapi_v1_msgs__msg__DoorLayout;
    is_plain =
      (
...
static size_t _DoorLayout__max_serialized_size(char & bounds_info)
{
  bool full_bounded;
  bool is_plain;
  size_t ret_val;

  ret_val = max_serialized_size_autoware_adapi_v1_msgs__msg__DoorLayout(
