...
빌드가 끝나면 홈 디렉토리 prefix 대신 아래 순서로 이 저장소 안 setup만 source한다.
```bash
...
```bash
source $BASE_DIR/msg_ws/install/local_setup.bash
source $BASE_DIR/deps_ws/install/local_setup.bash
source $BASE_DIR/bridge_ws/install/local_setup.bash
source $BASE_DIR/autoware_carla_interface/install/local_setup.bash
```
